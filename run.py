...
def input_type():
    ...
    """
    while True:
        print('Would you like to add a new item or'
              ' update sales figures or delete an entry?')
        data_str = input(
...
def update(data):
    if data in new:
        ...
        if item_name in first_col_items:
            ...
            stock_to_update = [item_name, item_code, sales, min_stock,
                               stock_on_hand, to_make, total_profit]
        ...
        item_name = input('Please enter the product name of what you'
                          ' would like to remove from your store: \n')
        df = SHEET.worksheet('items')
        first_col_items = df.col_values(1)
        if item_name in first_col_items:
            df = pd.DataFrame(items_data)
